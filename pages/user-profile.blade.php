    ...
    <div class="sectionHeading my-5 d-flex">
        <div class="container">
            <div class="row justify-content-between">
                <div class="col-5 ">
                    <h5 class="">ملف تعريفي للمستخدم</h5>
                </div>
                <div class="col-7 ">
                    <button type="button" class="btn add_wallet_btn position-relative float-start" data-bs-toggle="modal" data-bs-target="#exampleModal10">
                      أضف المحفظة
                    </button>
                    <button type="button" class="btn position-relative wallet_btn float-start">
					...
					@csrf	
                          ...
                          <div class="modal-body">
                            <div class="section_BG">
                              <h1 class="modal-title fs-5 text-center" id="exampleModalLabel">أضف المحفظة</h1>
                            ...
                            <div class="row mt-3 donar_detailCheckbox">
                              <p class="panel_titleDetail mb-1" style="font-size: 16px; font-weight: 700; color: #6d6c6c;">اختار طريقة الدفع:</p>
                              <ul>
                                  <li>
                          ...
                          <div class="modal-footer justify-content-center">
                            <div class="text-center">
                              <button type="submit" class="btn btn-green">أضف المحفظة</button>
                            </div>
                          </div>
                          </form>
                        </div>
    ...
    <div class="container">
      <div class="row ">
          <div class="user_profile_List align-items-start">
            <div class="col-md-3 tabBox ">
              <div class="nav nav-pills user_profile_List_arange " id="v-pills-tab" role="tablist" aria-orientation="vertical">
                <button class="nav-link active" id="v-pills-donationHistory-tab" data-bs-toggle="pill" data-bs-target="#v-pills-donationHistory" type="button" role="tab" aria-controls="v-pills-donationHistory" aria-selected="true">تاريخ التبرع</button>
                <button class="nav-link" id="v-pills-recurringDonation-tab" data-bs-toggle="pill" data-bs-target="#v-pills-recurringDonation" type="button" role="tab" aria-controls="v-pills-recurringDonation" aria-selected="false">التبرع المتكرر</button>
                <button class="nav-link" id="v-pills-sharedLinks-tab" data-bs-toggle="pill" data-bs-target="#v-pills-sharedLinks" type="button" role="tab" aria-controls="v-pills-sharedLinks" aria-selected="false">الروابط المشتركة</button>
                <button class="nav-link" id="v-pills-accountManagement-tab" data-bs-toggle="pill" data-bs-target="#v-pills-accountManagement" type="button" role="tab" aria-controls="v-pills-accountManagement" aria-selected="false">ادارة الحساب</button>
                <button class="nav-link" ><a href="{{ route('logout',app()->getLocale()) }}" >خروج</a></button>
              </div>
            </div>
            <div class="col-md-9 tabBox px-2 py-2 me-3">
             <div class="tab-content" id="v-pills-tabContent">
                 ...
                 <table class="table">
                    <thead>
                      <tr>
                        ...
                        <th scope="col">اسم ف</th>
                        <th scope="col">فئة</th>                     
                        <th scope="col">رقم المعاملة</th>
                        <th scope="col-sm-3">نوع الدفع</th>
                        <th scope="col">هبة</th>
                        <th scope="col">حالة</th>
                        <th scope="col">مخلوق</th>
                      </tr>
                    </thead>
                    <tbody>
                  ...
                  @if(!$alluserdonation->isEmpty())
                    ...
                    <tr>
                      ...
                      @endforeach
                    @else
                     <tr><td colspan="8"> ملحوظة: المعاملة فارغة؟.</td></tr>
                    @endif
                    </tbody>
                  </table>
                </div>
                ...
                <div class="table-responsive">
                  ...
                  @if(!$alluserrecurring_donation->isEmpty())
                    ...
                    <tr>
                      ...
                      @endforeach
                    @else
                     <tr><td  colspan="8"> ملاحظة: المعاملة المتكررة فارغة؟.</td></tr>
                    @endif
                    </tbody>
                  </table>
                   </div>
                ...
                <div class="table-responsive">
                    <table class="table">
                       <thead>
                         <tr>
                           <th scope="col">قم سري</th>
                           <th scope="col">وصلة</th>
                           <th scope="col">الزائرين</th>
                           <th scope="col">إجمالي التبرعات</th>                      
                         </tr>
                       </thead>
                       <tbody>
                    ...
                    @foreach($markertlinks as $tran)   
                      ...
                      @endforeach
                    @else
                     <tr><td  colspan="8"> ملاحظة: الرابط المشترك فارغ؟.</td></tr>
                    @endif
			       ...
			       @csrf
                    ...
                    <div class="mb-3">
                      ...
                      <input type="text" class="form-control" id="phone" name="phone" value="{{$userdata->phone}}" required>
                       @if ($errors->has('phone'))
                                <span style="width: 100%;" class="text-danger">{{ $errors->first('phone') }}</span>
                            @endif 
                    </div>
                    <div class="mb-3">
                      <label class="form-label d-block">النشرة الإخبارية</label>
                    ...
                    <div class="mb-3">
                        <label for="exampleFormControlTextarea1" class="form-label">عنوان</label>
                    ...
                    <h6 class="pb-2 pt-3">تغيير كلمة المرور</h6>
                    <form autocomplete="off" action="{{ route('updateuser-password',app()->getLocale()) }}" method="post">	
			           @csrf
                        <div class="mb-3">
                            <label for="exampleInputPassword1" class="form-label">كلمة المرور القديمة</label>
                            <input type="password" class="form-control" id="old_password" name="old_password" required>
                             @if ($errors->has('old_password'))
                                <span style="width: 100%;" class="text-danger">{{ $errors->first('old_password') }}</span>
                            @endif 
                        </div>
                        <div class="mb-3">
                            <label for="exampleInputPassword1" class="form-label">كلمة المرور الجديدة</label>
                            <input type="password" class="form-control" id="new_password" name="new_password" required>
                             @if ($errors->has('new_password'))
                                <span style="width: 100%;" class="text-danger">{{ $errors->first('new_password') }}</span>
                            @endif 
                        </div>
                        <div class="text-center">
                            <button type="submit" class="btn btn-green">تغيير كلمة المرور</button>
                        </div> 
                  </form>
                 <!-- ===================  -->
                </div>
